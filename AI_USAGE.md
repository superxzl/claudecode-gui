...
| 400 与 stdin warning 同时出现 | 推理网关拒绝请求体时，CLI 还会把 `-p` 模式的 stdin 等待提示写入 stderr | 过滤无关 stdin advisory，只保留真实 HTTP 400/502 或模型错误；实测全新 session、无工具和 safe mode 仍返回 400，确认当前阻塞在 `api.gosql.cn` 上游 |
